 ...
 */
define(["vue", 'select2', 'paging', 'datepicker', "jquery", "order"], function (Vue, select2, paging, dp, $) {
    return {
        init: function () {
            window.vm = new Vue({
                el: "#app",
                ...
                mounted: function () {
                    ...
                    //加载用户组信息
                    this.$http.get("/lib/user/groups.json", {params: {open: true}}).then(function (response) {
                        //渲染
                ...
                methods: {
                    queryData: function (index) {
                        ...
                        var layer_i = this.$layer.load();
                        this.$http.get("/lib/user/users.json", {params: params}).then(function (response) {
                            //判断返回值
                            if (response.body.code == 1) {
